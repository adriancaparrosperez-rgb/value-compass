...
def test_normalize_ticker(
    ...
    assert _normalize_ticker(value) == expected
...
def test_invalid_timestamp_returns_none(
    ...
    assert _unix_timestamp_to_iso(value) is None
...
def test_provider_builds_complete_snapshot(
    ...
    assert snapshot.fcf_yield == pytest.approx(
        60_000_000.0 / 1_010_000_000.0
    )
    assert snapshot.earnings_yield == pytest.approx(
        50_000_000.0 / 1_010_000_000.0
    )
    ...
    assert metadata["provider_role"] == "preload"
    assert metadata["is_official_source"] is False
    assert metadata["initialization"]["status"] == "success"
    assert metadata["normalization"]["status"] == "success"
    assert metadata["fast_info_usable"] is True
    assert metadata["info_usable"] is True
    assert metadata["provider_status"] == "success"
    assert metadata["contributing_components"] == [
        "fast_info",
        "info",
    ]
    assert metadata["price_source"] == "fast_info.last_price"
    assert metadata["market_cap_source"] == "fast_info.market_cap"
    assert metadata["validity_status"] == "assessed"
    assert metadata["consistency_status"] == "assessed"
...
def test_provider_falls_back_to_info_market_data(
    ...
    assert (
        snapshot.provider_metadata["price_source"]
        == "info.currentPrice"
    )
    assert (
        snapshot.provider_metadata["market_cap_source"]
        == "info.marketCap"
    )
    assert snapshot.provider_metadata["fast_info_usable"] is False
    assert snapshot.provider_metadata["info_usable"] is True
    assert snapshot.provider_metadata["provider_status"] == "partial"
...
def test_provider_preserves_zero_fcf(
    ...
    assert "free_cash_flow" not in snapshot.missing_fields
...
def test_provider_preserves_zero_net_income(
    ...
    assert "net_income" not in snapshot.missing_fields
...
def test_invalid_analyst_count_does_not_break_snapshot(
    ...
    info = _valid_info()
    info["numberOfAnalystOpinions"] = "N/A"
...
def test_fast_info_failure_allows_info_fallback(
    ...
    assert snapshot.market_cap == 1_000_000_000.0
    assert snapshot.errors == ""
    assert metadata["fast_info_error"] is True
    assert metadata["fast_info_usable"] is False
    assert metadata["info_usable"] is True
    assert metadata["provider_status"] == "partial"
    assert metadata["fast_info"]["exception_type"] == "RuntimeError"
    assert metadata["fast_info"]["message"] == "fast_info unavailable"

    assert any(
        "datos rápidos" in warning.casefold()
...
def test_info_failure_allows_fast_info_snapshot(
    ...
    assert metadata["info_error"] is True
    assert metadata["info_usable"] is False
    assert metadata["fast_info_usable"] is True
    assert metadata["provider_status"] == "partial"
    assert metadata["info"]["exception_type"] == "RuntimeError"
    assert metadata["info"]["message"] == "info unavailable"

    assert any(
        "ficha fundamental" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_fast_info_object_can_be_loaded_but_unusable(
    ...
    assert metadata["fast_info_available"] is True
    assert metadata["fast_info_usable"] is False
    assert metadata["fast_info"]["loaded"] is True
    assert metadata["fast_info"]["status"] == "unusable"
    assert metadata["fast_info"]["useful_field_count"] == 0
...
def test_fast_info_nan_values_are_not_usable(
    ...
    assert snapshot.provider_metadata["fast_info_usable"] is False
    assert snapshot.provider_metadata["provider_status"] == "failed"
    assert snapshot.price is None
    assert snapshot.market_cap is None
    assert snapshot.errors
...
def test_non_empty_info_without_usable_fields_is_rejected(
    ...
    assert metadata["info_available"] is True
    assert metadata["info_usable"] is False
    assert metadata["info"]["status"] == "unusable"
    assert metadata["provider_status"] == "failed"
    assert snapshot.errors
...
def test_both_sources_missing_return_controlled_error(
    ...
    assert snapshot.provider_metadata["provider_status"] == "failed"
    assert (
        snapshot.provider_metadata["contributing_component_count"]
        == 0
    )
...
def test_ticker_creation_failure_returns_snapshot(
    ...
    assert (
        snapshot.provider_metadata["initialization"]["status"]
        == "error"
    )
    assert (
        snapshot.provider_metadata["initialization"]["exception_type"]
        == "RuntimeError"
    )
    assert (
        snapshot.provider_metadata["initialization"]["message"]
        == "creation failure"
    )
    assert snapshot.provider_metadata["provider_status"] == "failed"
    assert snapshot.data_quality == 0.0
...
def test_missing_price_is_critical(
    ...
    assert snapshot.price is None
    assert "price" in snapshot.critical_missing_fields
...
def test_missing_market_cap_caps_quality(
    ...
    assert snapshot.market_cap is None
    assert "market_cap" in snapshot.critical_missing_fields
...
def test_market_cap_consistency_is_recorded(
    ...
    info = _valid_info()
    info["sharesOutstanding"] = 10_000_000.0

    fast_info = _valid_fast_info()
    fast_info["last_price"] = 100.0
    fast_info["market_cap"] = 1_000_000_000.0
    ...
    assert snapshot.consistency_score == 100.0
    assert (
        snapshot.provider_metadata["consistency_status"]
        == "assessed"
    )
    assert (
        snapshot.provider_metadata["market_cap_consistency_status"]
        == "assessed"
    )
    assert (
        snapshot.provider_metadata["market_cap_relative_difference"]
        == 0.0
    )
...
def test_large_market_cap_difference_generates_warning(
    ...
    info = _valid_info()
    info["sharesOutstanding"] = 1_000_000.0
    ...
    assert any(
        "capitalización no coincide" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_moderate_market_cap_difference_reduces_consistency(
    ...
    info = _valid_info()
    info["sharesOutstanding"] = 7_000_000.0
    ...
    assert any(
        "diferencia relevante" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_extreme_percentage_field_generates_warning(
    ...
    assert "roe" in snapshot.provider_metadata["invalid_fields"]
    assert snapshot.validity_score == 85.0
    assert any(
        "roe presenta un valor extremo" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_validity_is_not_assessable_without_checkable_metrics(
    ...
    assert snapshot.validity_score == 0.0
    assert (
        snapshot.provider_metadata["validity_status"]
        == "not_assessable"
    )
    assert (
        snapshot.provider_metadata["validity_evaluated_check_count"]
        == 0
    )
...
def test_negative_current_ratio_is_discarded(
    ...
    assert (
        "current_ratio"
        in snapshot.provider_metadata["invalid_fields"]
    )
    assert any(
        "current ratio es negativo" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_extreme_analyst_target_generates_warning(
    ...
    assert any(
        "precio objetivo" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_missing_fundamentals_date_generates_warning(
    ...
    assert any(
        "fecha fiable" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_successful_components_record_useful_fields(
    ...
    fast_metadata = snapshot.provider_metadata["fast_info"]
    info_metadata = snapshot.provider_metadata["info"]

    assert fast_metadata["status"] == "success"
    assert fast_metadata["usable"] is True
    assert fast_metadata["useful_fields"] == [
        "last_price",
        "market_cap",
        "last_price_time",
    ]
    assert (
        fast_metadata["useful_field_count"]
        == len(fast_metadata["useful_fields"])
    )
    assert "last_price" in fast_metadata["available_keys"]
    assert "market_cap" in fast_metadata["available_keys"]

    assert info_metadata["status"] == "success"
    assert info_metadata["usable"] is True
    assert "currentPrice" in info_metadata["useful_fields"]
    assert "marketCap" in info_metadata["useful_fields"]
    assert "totalRevenue" in info_metadata["useful_fields"]
    assert (
        info_metadata["useful_field_count"]
        == len(info_metadata["useful_fields"])
    )
...
def test_info_with_invalid_format_is_rejected(
    ...
    assert info_metadata["loaded"] is True
    assert info_metadata["status"] == "invalid_format"
    assert info_metadata["returned_type"] == "list"
    assert snapshot.provider_metadata["info_available"] is False
    assert snapshot.provider_metadata["info_usable"] is False
    assert snapshot.provider_metadata["fast_info_usable"] is True
    assert snapshot.provider_metadata["provider_status"] == "partial"
    assert snapshot.price == 101.0
    assert snapshot.errors == ""

    assert any(
        "formato no válido" in warning.casefold()
        for warning in snapshot.warnings
    )
...
def test_none_fast_info_is_recorded_as_empty(
    ...
    assert fast_metadata["loaded"] is True
    assert fast_metadata["status"] == "empty"
    assert fast_metadata["usable"] is False
    assert snapshot.provider_metadata["fast_info_available"] is False
    assert snapshot.provider_metadata["fast_info_usable"] is False
    assert snapshot.provider_metadata["info_usable"] is True
    assert snapshot.provider_metadata["provider_status"] == "partial"
    assert snapshot.price == 100.0
...
def test_none_info_is_recorded_as_invalid_format(
    ...
    assert info_metadata["loaded"] is True
    assert info_metadata["status"] == "invalid_format"
    assert info_metadata["returned_type"] == "NoneType"
    assert snapshot.provider_metadata["info_usable"] is False
    assert snapshot.provider_metadata["fast_info_usable"] is True
    assert snapshot.provider_metadata["provider_status"] == "partial"
    assert snapshot.name == "TEST"
...
def test_fast_info_price_date_has_priority(
    ...
    assert (
        snapshot.provider_metadata["price_observation_complete"]
        is True
    )
...
def test_info_price_date_is_used_as_fallback(
    ...
    info = _valid_info()
    fast_info = _valid_fast_info()
    fast_info["last_price_time"] = None
...
def test_exception_message_is_sanitized_and_limited(
    ...
    assert len(stored_message) <= 500
    assert (
        snapshot.provider_metadata["info"]["exception_type"]
        == "RuntimeError"
    )
...
def test_fast_info_object_with_get_method_is_supported(
    ...
    assert snapshot.provider_metadata["fast_info_usable"] is True
    assert (
        snapshot.provider_metadata["fast_info"]["status"]
        == "success"
    )
    assert (
        snapshot.provider_metadata["fast_info"][
            "useful_field_count"
        ]
        == 3
    )
    assert (
        "last_price"
        in snapshot.provider_metadata["fast_info"][
            "available_keys"
        ]
    )
