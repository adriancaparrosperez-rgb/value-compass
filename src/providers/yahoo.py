...
PROVIDER_SOURCE = "Yahoo Finance (precarga; fuente secundaria)"
...
def _normalize_ticker(ticker: Any) -> str:
    ...
    if not isinstance(ticker, str):
        raise ValueError("El ticker debe ser una cadena de texto.")
    ...
    if len(normalized_ticker) > 30:
        raise ValueError("El ticker no puede superar 30 caracteres.")
    if any(character.isspace() for character in normalized_ticker):
...
def _num(value: Any) -> float | None:
    """Convierte a float y rechaza booleanos, NaN e infinitos."""
...
def _integer(value: Any) -> int | None:
    """Convierte a entero no negativo solo si el valor es realmente entero."""
...
def _text(
    ...
    if len(normalized_value) > maximum_length:
        return normalized_value[: maximum_length - 3] + "..."

    return normalized_value


def _safe_get(container: Any, key: str) -> Any:
    """Obtiene un valor sin dejar que un campo defectuoso rompa la carga."""
...
def _first_not_none(*values: Any) -> Any:
    """Devuelve el primer valor distinto de None, conservando los ceros."""
    for value in values:
...
def _ratio(numerator: Any, denominator: Any) -> float | None:
    normalized_numerator = _num(numerator)
    normalized_denominator = _num(denominator)
    ...
    result = normalized_numerator / normalized_denominator
    return result if math.isfinite(result) else None


def _exception_details(error: Exception) -> dict[str, str]:
    """Devuelve detalles serializables y limitados del error."""
...
def _available_keys(container: Any) -> list[str]:
    """Obtiene una lista limitada de claves para diagnóstico."""
    try:
        if isinstance(container, Mapping):
    ...
    try:
        for candidate in candidates:
            normalized_key = _text(candidate, maximum_length=100)
            comparison_key = normalized_key.casefold()

            if not normalized_key or comparison_key in seen:
                continue
...
def _fast_info_useful_fields(fast_info: Any) -> list[str]:
    useful_fields: list[str] = []

    if _positive_or_none(_safe_get(fast_info, "last_price")) is not None:
        useful_fields.append("last_price")
    if _positive_or_none(_safe_get(fast_info, "market_cap")) is not None:
        useful_fields.append("market_cap")
    if (
        _unix_timestamp_to_iso(
            _safe_get(fast_info, "last_price_time")
        )
        is not None
    ):
        useful_fields.append("last_price_time")

    return useful_fields


def _info_useful_fields(info: Mapping[str, Any]) -> list[str]:
    useful_fields: list[str] = []
    ...
    for field_name in text_fields:
        if _text(
            _safe_get(info, field_name),
            maximum_length=250,
        ):
            useful_fields.append(field_name)
    ...
    for field_name in numeric_fields:
        if _num(_safe_get(info, field_name)) is not None:
            useful_fields.append(field_name)
    ...
    if (
        ...
        useful_fields.append("numberOfAnalystOpinions")
    ...
    return sorted(set(useful_fields))
...
def _load_fast_info(
    ...
    snapshot.provider_metadata["fast_info"] = component_metadata

    try:
        fast_info = yahoo_ticker.fast_info
    except Exception as error:
        logger.exception(
            "No se pudo cargar fast_info de Yahoo para %s.",
            snapshot.ticker,
        )
        ...
        snapshot.provider_metadata["fast_info_error"] = True
        snapshot.provider_metadata["fast_info_available"] = False
        snapshot.provider_metadata["fast_info_usable"] = False
        snapshot.add_warning(
            "Yahoo no pudo proporcionar los datos rápidos del mercado."
        )
        return {}

    component_metadata["loaded"] = True

    if fast_info is None:
        component_metadata["status"] = "empty"
        snapshot.provider_metadata["fast_info_available"] = False
        snapshot.provider_metadata["fast_info_usable"] = False
        snapshot.add_warning(
    ...
    useful_fields = _fast_info_useful_fields(fast_info)
    is_usable = bool(useful_fields)
    ...
    component_metadata.update(
        {
            ...
            "usable": is_usable,
            "available_keys": _available_keys(fast_info),
            "useful_fields": useful_fields,
            "useful_field_count": len(useful_fields),
        }
    )
    snapshot.provider_metadata["fast_info_available"] = True
    snapshot.provider_metadata["fast_info_usable"] = is_usable
...
def _load_info(
    ...
    snapshot.provider_metadata["info"] = component_metadata
    ...
    try:
        ...
        snapshot.provider_metadata["info_error"] = True
        snapshot.provider_metadata["info_available"] = False
        snapshot.provider_metadata["info_usable"] = False
        snapshot.add_warning(
        ...
        component_metadata.update(
            {
                ...
                "returned_type": type(info).__name__,
            }
        )
        snapshot.provider_metadata["info_available"] = False
        snapshot.provider_metadata["info_usable"] = False
    ...
    useful_fields = _info_useful_fields(info)
    is_available = bool(info)
    is_usable = bool(useful_fields)
    ...
    component_metadata.update(
        {
            ...
            "usable": is_usable,
            "available_keys": _available_keys(info),
            "useful_fields": useful_fields,
            "useful_field_count": len(useful_fields),
        }
    )
    snapshot.provider_metadata["info_available"] = is_available
    snapshot.provider_metadata["info_usable"] = is_usable
...
def _assign_market_fields(
    ...
    snapshot.provider_metadata["price_source"] = price_source
    ...
    snapshot.enterprise_value = _positive_or_none(
        _safe_get(info, "enterpriseValue")
    )
    snapshot.fifty_two_week_change = _num(
        _safe_get(info, "52WeekChange")
    )
    snapshot.analyst_target = _positive_or_none(
        _safe_get(info, "targetMeanPrice")
    )
    snapshot.analyst_count = _integer(
        _safe_get(info, "numberOfAnalystOpinions")
    )
    snapshot.price_date = _first_not_none(
        _unix_timestamp_to_iso(
            _safe_get(fast_info, "last_price_time")
        ),
        _unix_timestamp_to_iso(
    ...
    snapshot.provider_metadata[
        ...
        and snapshot.price_date is not None
    )
...
def _assign_fundamental_fields(
    ...
    snapshot.revenue = _num(
        _safe_get(info, "totalRevenue")
    )
    snapshot.ebitda = _num(
        _safe_get(info, "ebitda")
    )
    snapshot.ebit = _num(
        _safe_get(info, "ebit")
    )
    snapshot.net_income = _num(
        _safe_get(info, "netIncomeToCommon")
    )
    ...
    snapshot.capex = None
    snapshot.total_cash = _non_negative_or_none(
        _safe_get(info, "totalCash")
    )
    snapshot.total_debt = _non_negative_or_none(
        _safe_get(info, "totalDebt")
    )
    ...
    snapshot.forward_pe = _positive_or_none(
    ...
    snapshot.fundamentals_date = (
        _unix_timestamp_to_iso(
            _safe_get(info, "mostRecentQuarter")
        )
    )
...
def _validate_value_ranges(
    ...
    if snapshot.current_ratio is not None:
        ...
        if snapshot.current_ratio < 0:
            invalid_fields.append("current_ratio")
            snapshot.add_warning(
...
def _calculate_coverage_score(
    ...
    total_weight = sum(
        weight
        for _, weight in weighted_fields.values()
    )
...
def _calculate_quality_scores(
    ...
    snapshot.coverage_score = (
        _calculate_coverage_score(snapshot)
    )
    validity_assessed = (
...
def _finalize_snapshot(
    ...
    if snapshot.capex is None:
        snapshot.add_warning(
            "El CAPEX no se obtiene de forma fiable "
            "en esta precarga y permanece sin evaluar."
        )
    ...
    if snapshot.fundamentals_date is None:
        snapshot.add_warning(
            "Yahoo no proporcionó una fecha fiable "
            "para los fundamentales."
        )
    if (
        snapshot.provider_metadata.get(
            "validity_status"
        ...
        snapshot.add_warning(
            "La validez no pudo evaluarse por falta "
            "de métricas aptas para los controles disponibles."
        )
    if (
        snapshot.provider_metadata.get(
            "consistency_status"
        ...
        snapshot.add_warning(
            "La consistencia entre precio, acciones "
            "y capitalización no pudo evaluarse."
        )
...
class YahooProvider(
    ...
    def get_snapshot(
        ...
        try:
            ...
            logger.exception(
                "Error inesperado al normalizar "
                "datos de Yahoo para %s.",
                normalized_ticker,
            )
            snapshot.provider_metadata[
                "normalization"
